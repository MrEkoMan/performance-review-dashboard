...
function Dashboard() {
    ...
    async function loadEvidenceRecency() {
        try {
            const data = await getEvidenceRecency();
            setEvidenceRecency(Array.isArray(data) ? data : []);
        } catch (err) {
            console.error("loadEvidenceRecency failed:", err);
            setError(err.message);
        }
    }
